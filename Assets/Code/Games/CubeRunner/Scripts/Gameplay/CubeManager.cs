...
{
    internal class CubeManager: BaseDisposable
    {
        ...
        private void OnPhysicUpdate(float deltaTime)
        {
            if (_isRotating)
            {
                ...
                float ratio = Mathf.Lerp(0, 1, _rotationTime / _duration);

                float rotAng = Mathf.Lerp(0, Mathf.PI / 2f, ratio);
                float distanceX = _currentDirection.x * _radius * (Mathf.Cos(_startAngleRad) - Mathf.Cos(_startAngleRad + rotAng));
                float distanceY = _radius * (Mathf.Sin(_startAngleRad + rotAng) - Mathf.Sin(_startAngleRad));
                float distanceZ = _currentDirection.y * _radius * (Mathf.Cos(_startAngleRad) - Mathf.Cos(_startAngleRad + rotAng));
        ...
        public CubeView SpawnCube(Vector3 scale, Vector3 position)
        {
            ...
            if (cubePrefabObject != null)
            {
                ...
                GameObject cubeObject = _poolManager.Get(cubePrefabObject,spawnPos, parent, Quaternion.identity);
                if (cubeObject != null)
                {
        ...
        void SetRadius(Vector2Int direction)
        {
            const float threshold = 0.99f;

            Vector3 moveDir = direction.x != 0 
                ? Vector3.right 
                : direction.y != 0 
                    ? Vector3.forward 
                    : Vector3.zero;
            ...
            foreach (var r in rules)
            {
                ...
                {
                    float a = r.Value.a * 0.5f;
                    float b = r.Value.b * 0.5f;
