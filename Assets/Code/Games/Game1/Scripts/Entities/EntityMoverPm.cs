...
{
internal class EntityMoverPm : DisposableBase
{
	internal struct Ctx
	{
	...
	protected virtual Vector2 GetInput()
	{
		...
		var joystickDirection = _inputManager.GetJoystickInput();
		...
		var targetAngle = Mathf.Atan2(joystickDirection.y, joystickDirection.x) * Mathf.Rad2Deg;
	...
	private void FixedUpdate(float deltaTime)
	{
		...
		void UpdateRotateSpeed()
		{
			...
			if (_ctx.useAcceleration)
			{
				...
				var acceleration = requiredRotateSpeed > currentRotateSpeed
					? _ctx.model.AccelerationRotateSpeed.Value
					: -1 * _ctx.model.DecelerationRotateSpeed.Value;
				currentRotateSpeed += acceleration * deltaTime;
				currentRotateSpeed = Mathf.Clamp(currentRotateSpeed, -_ctx.model.MaxRotateSpeed.Value,
					_ctx.model.MaxRotateSpeed.Value);
			}
			else
			{
		...
		void UpdateMoveSpeed()
		{
			...
			if (_ctx.useAcceleration)
			{
				...
				var acceleration = requiredSpeed > currentSpeed
					? _ctx.model.AccelerationSpeed.Value
					: -1 * _ctx.model.DecelerationSpeed.Value;
