    ...
    public class QueueShortGamesLoader : IGamesLoader
    {
        ...
        public async ValueTask<IShortGame> LoadGameAsync(Type gameType, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    _logger.Log($"Game {gameType.Name} is already loaded");
                    return existingGame;
                }
        ...
        public async ValueTask<IShortGame> PreloadGameAsync(Type gameType, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    _logger.Log($"Game {gameType.Name} is already loaded");
                    return _loadedGames[gameType];
                }
                
                if (_preloadedGames.ContainsKey(gameType))
                {
                    _logger.Log($"Game {gameType.Name} is already preloaded");
                    return _preloadedGames[gameType];
                }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < gameTypesList.Count; i++)
            {
                if (results[i] != null)
                {
