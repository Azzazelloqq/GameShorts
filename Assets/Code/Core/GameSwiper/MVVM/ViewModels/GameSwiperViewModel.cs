...
internal class GameSwiperViewModel : ViewModelBase<GameSwiperModel>
{
    ...
    private CancellationTokenSource _currentGameReadyCts;
	private CancellationTokenSource _preloadRefreshCts;
    ...
    protected override void OnDispose()
    {
        ...
        _currentGameReadyCts?.Cancel();
        _currentGameReadyCts?.Dispose();
        _currentGameReadyCts = null;

		_preloadRefreshCts?.Cancel();
		_preloadRefreshCts?.Dispose();
		_preloadRefreshCts = null;
		...
		_gameServiceProvider.OnGamePreloadingCompleted -= HandleGamePreloadingCompleted;
    ...
    private void InitializeSync()
    {
        ...
        compositeDisposable.AddDisposable(
            model.CurrentGame.Subscribe(game => OnSlotModelChanged(GameSlot.Current, game)));
    ...
    private async UniTask OnGoToNext()
    {
        ...
        var success = await NavigateAsync(
            ...
            () => _gameServiceProvider.NextGameType != null,
            1,
    ...
    private async UniTask OnGoToPrevious()
    {
        ...
        var success = await NavigateAsync(
            ...
            () => _gameServiceProvider.PreviousGameType != null,
            -1,
    ...
    private async UniTask WaitForGameReadyAsync(
        Func<bool> predicate,
        ...
        int maxWaitTimeMs = 10000)
    {
        ...
        while (!predicate() && (maxWaitTimeMs < 0 || waitTime < maxWaitTimeMs))
        {
    ...
    private static int ResolveTimeoutMs(TimeSpan timeout)
    {
    ...
    private async UniTask RefreshGamesFromServiceAsync(CancellationToken token)
    {
        ...
        var previousType = _gameServiceProvider.PreviousGameType;
        var currentType = _gameServiceProvider.CurrentGameType;
        var nextType = _gameServiceProvider.NextGameType;

        var previous = previousType != null
        ...
        var current = await BuildSlotStateAsync(
            currentIndex,
	...
	private void HandleGamePreloadingCompleted(Type gameType)
	{
		if (gameType == null)
		{
			return;
		...
		if (gameType != _gameServiceProvider.CurrentGameType &&
			gameType != _gameServiceProvider.NextGameType &&
			gameType != _gameServiceProvider.PreviousGameType)
		{
			return;
		}
		...
		_preloadRefreshCts?.Cancel();
		_preloadRefreshCts?.Dispose();
		_preloadRefreshCts = new CancellationTokenSource();
		_ = RefreshSlotsAfterPreloadAsync(_preloadRefreshCts.Token);
	}

	private async UniTask RefreshSlotsAfterPreloadAsync(CancellationToken token)
	{
		try
		{
			await RefreshGamesFromServiceAsync(token);
		...
		{
			// Expected when newer preload events arrive.
		}
	}
    ...
    private void BeginCurrentGameReadyWait()
    {
        _currentGameReadyCts?.Cancel();
        _currentGameReadyCts?.Dispose();
        _currentGameReadyCts = new CancellationTokenSource();
        _ = WaitForCurrentGameReadyAsync(_currentGameReadyCts.Token);
    }

    private async UniTask WaitForCurrentGameReadyAsync(CancellationToken token)
    {
        try
        {
            var timeoutMs = ResolveTimeoutMs(_gameServiceProvider.ReadinessTimeout);
            var fallbackAttempts = Math.Max(1, _gameServiceProvider.FallbackLoadAttempts);

            for (var attempt = 0; attempt <= fallbackAttempts; attempt++)
            {
                var wasReady = _gameServiceProvider.IsCurrentGameReady;
                ...
                await WaitForGameReadyAsync(() => _gameServiceProvider.IsCurrentGameReady, token, timeoutMs);
                ...
                {
                    _logger.LogError("Current game preload timed out. Cancelling loading indicator.");
                    model.SetLoadingState(false);
                    return;
                }

                var ensured = await _gameServiceProvider.EnsureCurrentGamePreloadedAsync(token);
                if (!ensured)
                {
                    _logger.LogWarning("Fallback preload attempt failed for current game.");
                }
            }
        }
        catch (OperationCanceledException)
        {
            // Expected when navigation changes or view model disposes.
        }
    }
    ...
    private void UpdateNeighbourPreviewRendering(float progress)
    {
        ...
        switch (direction)
        {
            case SwipeDirection.Up:
                _gameServiceProvider.SetNeighbourRenderingEnabled(true, false);
                break;
            case SwipeDirection.Down:
                _gameServiceProvider.SetNeighbourRenderingEnabled(false, true);
                break;
            default:
                _gameServiceProvider.SetNeighbourRenderingEnabled(false, false);
                break;
        }
    }
