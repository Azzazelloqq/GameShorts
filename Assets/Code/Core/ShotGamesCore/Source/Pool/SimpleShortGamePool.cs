...
public class SimpleShortGamePool : IShortGamesPool
{
	...
	public void ReleaseShortGame(IPoolableShortGame game)
	{
		...
		{
			_logger.LogWarning($"Pool for {gameType.Name} is full ({_maxInstancesPerType} instances). Destroying excess game.");
	...
	public void WarmUpPool(IPoolableShortGame game)
	{
		...
		if (gamesQueue.Count >= _maxInstancesPerType)
		{
			_logger.LogWarning($"Cannot warm up pool for {gameType.Name} - already at max capacity ({_maxInstancesPerType})");
